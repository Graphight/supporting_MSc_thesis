...
def process_data_into_regions_wah(file: str) -> None:
    try:
        ...
        region_map_file_name = f"{DATA_DIR_SIM}/region_map_HAPPI.json"
        ...
        for model_metric in model_metrics:
            ...
            df_temp: pd.DataFrame = ds_temp.to_dataframe().reset_index(drop=False)
        ...
        for region in sorted(df_joined["region"].unique()):
            ...
            if path.exists(output_filename):
                # The default behaviour here is to NOT overwrite existing files
                # This could be considered an "anti-pattern" as it relies on the user deleting bad generated data
                return None

            df_joined.loc[df_joined["region"] == region].reset_index(drop=True).drop(columns=["year", "region"]).to_parquet(output_filename)
...
DATA_DIR_SIM = f"{COMMON_DIR}/Data_WeatherAtHome/HAPPI"
...
if __name__ == "__main__":
    process_files("ALL", sorted(glob(f"{DATA_DIR_SIM}/raw_data/ALL/batch_860/region/item5216_daily_mean/*.nc")))
    process_files("HOT", sorted(glob(f"{DATA_DIR_SIM}/raw_data/HOT/batch_872/region/item5216_daily_mean/*.nc")))
